...
export default function SweetCard({ sweet, onUpdate, onDelete, onPurchase }) {
  const { user } = useAuth();
    ...
    >
      <div style={{ paddingBottom: '0.5rem', borderBottom: '1px solid #f3f4f6' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: '700', color: 'var(--text-main)' }}>{sweet.name}</h3>
            <span style={{ 
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: 'auto' }}>
        ...
        {isAdmin && (
            <>
                <button onClick={() => onUpdate(sweet)} className="btn btn-secondary" style={{ padding: '0.75rem' }}>
                    <Edit size={18} />
                </button>
                <button onClick={() => onDelete(sweet._id)} className="btn btn-secondary" style={{ padding: '0.75rem', color: 'var(--danger)', borderColor: '#fca5a5' }}>
                    <Trash2 size={18} />
                </button>
            </>
        )}
